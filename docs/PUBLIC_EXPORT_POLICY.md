...
4. Verify Android package/version identity, the read-only boundary, and build configuration.
5. Run public validators, tests, lint, and assembly where available.
6. Review the complete diff before merging.

## Provenance

Each source-import pull request states the exact private source commit from which it was derived, describes excluded paths or transformations, and records checks actually executed. The public commit is authoritative for public users; private operational history is not required to build or understand the public application.
