...
class T3SynchronousTuneSnapshotReaderTest {
    @Test
    fun readsEveryPlannedChunkAndBuildsExactImmutableSnapshot() {
        ...
        val reader = T3SynchronousTuneSnapshotReader(profile, context, chunkBytes = 2)
    ...
    @Test
    fun contextProfileFingerprintMismatchFailsBeforeAnyRead() {
        val profile = profile()
        val wrongContext = context(profile).copy(profileFingerprint = "0".repeat(64))
        assertThrows(IllegalArgumentException::class.java) {
            T3SynchronousTuneSnapshotReader(profile, wrongContext, chunkBytes = 2)
        }
    }

    @Test
    fun truncatedChunkFailsClosed() {
        val profile = profile()
        val reader = T3SynchronousTuneSnapshotReader(profile, context(profile), chunkBytes = 2)
        var calls = 0
        assertThrows(IllegalArgumentException::class.java) {
            reader.read(1234L) { _, _, _ ->
                calls++
                byteArrayOf(0x00)
            }
        }
    ...
    @Test
    fun nonzeroReadStatusFailsClosed() {
        val profile = profile()
        val reader = T3SynchronousTuneSnapshotReader(profile, context(profile), chunkBytes = 2)
        assertThrows(IllegalArgumentException::class.java) {
    ...
    @Test
    fun payloadIsExactlyTheAcceptedT1RangeCodec() {
        val profile = profile()
